...
public class DeleteRemittanceCommandHandler(
    SpheraDbContext dbContext,
    ILogger<DeleteRemittanceCommandHandler> logger)
    : IHandler<DeleteRemittanceCommand, bool>
{
    public async Task<IResultDTO<bool>> HandleAsync(
        DeleteRemittanceCommand request,
        HttpContext context,
        CancellationToken cancellationToken)
    {
        logger.LogInformation("Deletando remessa {Id}", request.Id);
        ...
        var result = await strategy.ExecuteAsync(async () =>
        {
            try
            {
                ...
                dbContext.Remittances.Remove(entity);
            ...
            {
                await dbContext.Database.RollbackTransactionAsync(cancellationToken);
                return ResultDTO<bool>.AsFailure(new FailureDTO(500, "Erro ao deletar remessa."));
            }
        });
