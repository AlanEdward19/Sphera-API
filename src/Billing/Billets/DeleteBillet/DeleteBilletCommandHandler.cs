...
namespace Sphera.API.Billing.Billets.DeleteBillet;

public class DeleteBilletCommandHandler(
    SpheraDbContext dbContext,
    ILogger<DeleteBilletCommandHandler> logger)
    : IHandler<DeleteBilletCommand, bool>
{
    public async Task<IResultDTO<bool>> HandleAsync(
        DeleteBilletCommand request,
        HttpContext context,
        CancellationToken cancellationToken)
    {
        logger.LogInformation("Deletando billet {Id}", request.Id);
        ...
        var result = await strategy.ExecuteAsync(async () =>
        {
            try
            {
                ...
                dbContext.Billets.Remove(entity);
            ...
            {
                await dbContext.Database.RollbackTransactionAsync(cancellationToken);
                return ResultDTO<bool>.AsFailure(new FailureDTO(500, "Erro ao deletar billet."));
            }
        });
