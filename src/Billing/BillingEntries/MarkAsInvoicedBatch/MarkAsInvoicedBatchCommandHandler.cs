...
public class MarkAsInvoicedBatchCommandHandler(
    SpheraDbContext dbContext,
    ILogger<MarkAsInvoicedBatchCommandHandler> logger)
    : IHandler<MarkAsInvoicedBatchCommand, BulkActionResultDTO>
{
    public async Task<IResultDTO<BulkActionResultDTO>> HandleAsync(
        MarkAsInvoicedBatchCommand request,
        HttpContext context,
        CancellationToken cancellationToken)
    {
        ...
        foreach (var entry in entities)
        {
            try
            {
                entry.MarkAsInvoiced(request.InvoiceId, actor);
