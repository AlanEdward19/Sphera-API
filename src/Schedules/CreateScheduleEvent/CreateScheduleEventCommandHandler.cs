...
public class CreateScheduleEventCommandHandler(SpheraDbContext dbContext, ILogger<CreateScheduleEventCommandHandler> logger)
    : IHandler<CreateScheduleEventCommand, ScheduleEventDTO>
{
    ...
    public async Task<IResultDTO<ScheduleEventDTO>> HandleAsync(CreateScheduleEventCommand request, HttpContext context, CancellationToken cancellationToken)
    {
        logger.LogInformation("Criando evento de agenda para UserId={UserId} ClientId={ClientId} OccurredAt={OccurredAt}",
        ...
        catch (Exception e)
        {
            ...
            return ResultDTO<ScheduleEventDTO>.AsFailure(new FailureDTO(500, "Ocorreu um erro ao tentar criar um evento para o usuário."));
        }
    }
}